...
class TestClass1 {
    ...
    // private가 아닌 접근 제한자를 붙힌 변수는 Java로 변환될 때 변수의 접근 제한자는 private로 변경된다
    // 이때, var로 선언된 변수 setter와 getter가 추가된다.
    public var value3 = 300
    // val로 선언된 변수는 getter만 추가된다.
    public val value4 = 400

    // 자동 생성되는 setter는 매개변수의 값을 그냥 멤버변수에 저장하는 코드로 되어 있다.
    // 자동 생성되는 getter는 멤버변수의 값을 반환하는 코드로 되어 있다.
    // 만약 자동 생성되는 setter나 getter를 사용하지 않고 직접 정의하고 싶다면 다음과 같이 작성해준다.
    var value5 = 500
        // field : 변수를 의미한다. (즉, value5의 변수)
        // getter
        get(){
            println("getter 호출")
            return field
