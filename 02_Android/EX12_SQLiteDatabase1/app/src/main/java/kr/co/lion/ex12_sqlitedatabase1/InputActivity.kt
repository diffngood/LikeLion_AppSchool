...
class InputActivity : AppCompatActivity() {
    ...
    fun setView(){
        activityInputBinding.apply {
            ...
            // 키보드를 올린다     이 때, View를 지정해야한다.
            showSoftInput(textFieldInputName)

            // (맨 마지막 수학점수 입력칸) 엔터키를 누르면 입력 완료 처리를 한다.
    ...
    fun processInputDone(){
        ...
        activityInputBinding.apply {
            ...
            if (korStr.isEmpty()){
                showDialog("국어 입력 오류", "국어 점수를 입력해주세요", textFieldInputKor)
                return
            }
            if (engStr.isEmpty()){
                showDialog("영어 입력 오류", "영어 점수를 입력해주세요", textFieldInputEng)
                return
            }
            if (mathStr.isEmpty()){
                showDialog("수학 입력 오류", "수학 점수를 입력해주세요", textFieldInputMath)
                return
            }

            // 데이터 베이스에 저장한다.
            // 여기서 idx는 쓰이지 않아서 아무거나 써도된다.
