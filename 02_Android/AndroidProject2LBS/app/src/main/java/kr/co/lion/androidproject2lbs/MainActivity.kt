                    ...
                    when(it.itemId){
                        ...
                        R.id.main_menu_place -> {
                            ...
                            materialAlertDialogBuilder.setItems(dialogData){ dialogInterface: DialogInterface, i: Int ->
                                // 주변 정보를 가져온다.
                                // 매개 변수에는 i 번째 (사용자가 선택한 항목의 순서 값) 문자열을 전달해준다.
                                getPlaceData(dialogData[i])
                            }
                            materialAlertDialogBuilder.show()
                        }
                        // 주변 정보 마커 초기화
                        R.id.main_manu_clear -> {
                            initPlaceData()
                        }
                    }
                    true
...
//            mainGoogleMap.mapType = GoogleMap.MAP_TYPE_HYBRID
            ...
            if (a1 && a2) {
                ...
                // 저장되어 있는 위치 정보 값을 가져온다. (없으면 null이 반환된다)
    ...
    fun getMyLocation(){
        ...
        // Network 프로바이더 사용이 가능하다면 (실내 테스트하고있어서 이렇게 원래면 else if 로 해도됨)
        if(locationManager.isProviderEnabled(LocationManager.NETWORK_PROVIDER) == true){
            Log.d("test1234", "NETWORK_PROVIDER : ${LocationManager.NETWORK_PROVIDER}")
            if(networkLocationListener == null) {
                Log.d("test1234", "NETWORK_PROVIDER 실행")
    ...
    fun setMyLocation(location: Location) {
        ...
        // 위도와 경도를 출력한다.
        val snack = Snackbar.make(activityMainBinding.root, "provider : ${location.provider}, 위도 : ${location.latitude}, 경도 : ${location.longitude}", Snackbar.LENGTH_SHORT)
        snack.setTextColor(Color.LTGRAY)	// 텍스트 컬러
        snack.setBackgroundTint(Color.BLACK)	// 배경 컬러
        snack.animationMode = Snackbar.ANIMATION_MODE_SLIDE	// 애니메이션
        snack.show()
        ...
        try {
            ...
            thread {
                ...
                if (userLocation != null) {
                    ...
                    val language = "ko"
                    // API 키
                    val apiKey = "AIzaSyB_Idc43Skb7_U8cYHNmwRrdSPc3tiDKi4"
                    ...
                    do {
                        ...
                        val serverPath = if (type == "all") {
                            ...
                            "${site}?location=${location}&radius=${radius}&language=${language}&type=${type}&key=${apiKey}"
                        }
                        ...
                        // 전체가 { } 이므로 JSONObject를 생성한다.
                        val root = JSONObject(stringBuffer.toString())

                        // Log.d("test1234", "${root.has("status")}") has는 true / flase
                        // status 값이 OK 인 경우에만 데이터를 가져온다
                        if (root.has("status")){
                            val status = root.getString("status")
                            if (status == "OK") {
                                // results라는 이름의 배열을 가져온다.
                                if (root.has("results")){
                                ...
                                if (root.has("next_page_token")){
                                    ...
                                    Log.d("test1234", "이번 페이지 끝")
                                } else {
                                    // null을 넣어준다.
                                    // do while의 조건식이 토큰이 null 이 아닐 경우 이므로
                                    // null을 넣어서 반복을 중단시킨다.
                                    pagetoken = null
                                }
                            } else {
                                showDataError()
                                pagetoken = null
                                Log.d("test1234", "Now status is INVALID_REQUEST")
                            }
                        } else {
                            showDataError()
                        }
    ...
    // 지도에 표시되어 있는 마커를 제거하고 모든 리스트를 초기화 한다.
    fun initPlaceData(){
