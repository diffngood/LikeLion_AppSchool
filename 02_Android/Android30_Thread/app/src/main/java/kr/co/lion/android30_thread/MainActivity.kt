...
// 그 안에 작성된 작업을 처리하기 위해 안드로이드 OS가 쓰레드를 발생시킨다.
...
// UI Thread, original Thread : 안드로이드 OS가 발생시킨 쓰레드. 화면 갱신, 생명주기와 관련된 메서드 호출, 리스너 처리 등의 일을
// 담당하게 된다. 이 쓰레드가 호출하는 메서드 내부의 코드는 무조건 금방 끝나야 한다.
// 사용자 쓰레드 : 개발자가 thread 코드 블럭을 이용해 발생시키는 쓰레드. 오래 걸리는 작업이나, 오류가 발생될
...
class MainActivity : AppCompatActivity() {
    ...
    // 사용자 쓰레드는 Activity가 종료되었다고 하더라도 계속 동작을 한다.
    // OS가 판단했을 때 시스템 메모리가 부족한 경우 사용자 쓰레드들이 모두 종료된다. (이를 방지하기 위해서는 Service를 사용해야 한다)
    // 따라서 Activity가 종료될 때 발생시킨 쓰레드들도 종료되게 해야 한다.
