...
// 3. MainActivity.kt 에 oldFragment와 newFragment 프로퍼티를 정의해준다.
// 4. fragment basic code.txt 에 있는 메서드 코드를 붙혀 넣는다.
...
class MainActivity : AppCompatActivity() {
    ...
    fun replaceFragment(name:FragmentName, addToBackStack:Boolean, isAnimate:Boolean, data:Bundle?){
        ...
        when(name){
            ...
            FragmentName.SHOW_INFO_FRAGMENT -> {
                newFragment = ShowInfoFragment()
            }
        }
...
enum class FragmentName(var str:String){
    ...
    SHOW_INFO_FRAGMENT("ShowInfoFragment"),
}
