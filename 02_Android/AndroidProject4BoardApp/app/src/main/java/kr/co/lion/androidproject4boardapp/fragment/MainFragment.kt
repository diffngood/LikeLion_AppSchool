...
class MainFragment : Fragment() {
    ...
    inner class MainRecyclerViewAdapter: RecyclerView.Adapter<MainRecyclerViewAdapter.MainViewHolder>(){
        ...
        override fun onBindViewHolder(holder: MainViewHolder, position: Int) {
            ...
            holder.rowMainBinding.root.setOnClickListener {
                // 필요한 데이터를 담아준다.
                val readBundel = Bundle()
                readBundel.putInt("contentIdx", mainList[position].contentIdx)
                // 글 읽는 화면으로 이동한다.
                contentActivity.replaceFragment(ContentFragmentName.READ_CONTENT_FRAGMENT, true, true, readBundel)
            }
        }
    }
    ...
    inner class SearchRecyclerViewAdapter: RecyclerView.Adapter<SearchRecyclerViewAdapter.SearchViewHolder>() {
        inner class SearchViewHolder(rowMainBinding: RowMainBinding) :
