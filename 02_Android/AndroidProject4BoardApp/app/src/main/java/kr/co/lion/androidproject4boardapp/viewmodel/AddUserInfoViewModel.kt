...
class AddUserInfoViewModel: ViewModel() {
    ...
    // 툴바의 타이틀
    val toolbarAddUserInfoTitle = MutableLiveData<String>()
    val toolbarAddUserInfoNavigationIcon = MutableLiveData<Int>()
    ...
    // 취미 전체
    val checkedBoxAddUserInfoAllState = MutableLiveData<Int>()
    ...
    fun onCheckBoxChanged(){
        ...
        if (checkedCnt == 0) {
            ...
            checkedBoxAddUserInfoAllState.value = MaterialCheckBox.STATE_UNCHECKED
        }
        // 모두 체크되어 있다면
        else if (checkedCnt == 6){
            checkBoxAddUserInfoAll.value = true
            checkedBoxAddUserInfoAllState.value = MaterialCheckBox.STATE_CHECKED
        }
        // 일부만 체크되어 있다면
        else {
            checkedBoxAddUserInfoAllState.value = MaterialCheckBox.STATE_INDETERMINATE
        }
    }

    companion object {
        // ViewModel에 값을 설정하여 화면에 반영하는 작업을 할 때 호출된다.
        // () 안에는 속성의 이름을 넣어준다. 속성의 이름은 자유롭게 해주면 되지만 기존의 속성 이름과 중복되지 않게 해줘야 한다.
        // 매개 변수: 값이 설정된 View 객체, ViewModel을 통해 설정되는 값
        @BindingAdapter("android:checkedButtonId") // build.gradle.kts(:app)에 플러그인에 id(kotlin-kapt) 추가
        @JvmStatic
