...
// - button5, button6 예제
// 외부 저장소의 Android/data를 제외한 모든 곳 : 애플리케이션에서 파일에 직접 접근하는 것이 불가능하다.
...
class MainActivity : AppCompatActivity() {
    ...
    // 파일 앱을 실행하기 위한 런처
    lateinit var writeActivityResultLauncher:ActivityResultLauncher<Intent>
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        writeActivityResultLauncher = registerForActivityResult(contract1){
            // 파일을 선택하면 resultCode는 RESULT_OK가 들어온다.
            if (it.resultCode == RESULT_OK) {
                // 사용자 선택한 파일의 정보를 가지고 있는 Intent로 부터 파일 정보를 가져온다.
                if (it.data != null) {
                    // 저장할 파일에 접근할 수 있는 객체로 부터 파일 정보를 받아온다.
                    // w : 쓰기, a : 이어쓰기, r : 읽기
        ...
        activityMainBinding.apply {
            ...
            button5.setOnClickListener {
                ...
                // MimeType을 설정해준다.
                // 파일에 저장되어 있는 데이터 양식이 무엇인지를 나타내는 문자열
                ...
                // 모든 소리 파일 : audio/*
                fileIntent.type = "*/*"
                writeActivityResultLauncher.launch(fileIntent)
            }
